...
const aiWaterClient_1 = require("../aiWaterClient");
...
let lastSensorData = { ...DEFAULT_SENSORS };
let lastAnomalyResult = null;
let lastLeakResult = null;
let lastTTFResult = null;
// ─── Health Check ──────────────────────────────────────────────────────────
router.get('/health', async (_req, res) => {
    try {
        const aiHealth = await aiWaterClient_1.aiWaterClient.healthCheck();
        res.json({
            backend: 'healthy',
            ai_model: aiHealth,
        });
...
router.post('/analyze', async (req, res) => {
    try {
        ...
        const [anomalyResult, ttfResult, leakResult] = await Promise.all([
            aiWaterClient_1.aiWaterClient.predictAnomaly(sensorData),
            aiWaterClient_1.aiWaterClient.predictTTF(sensorData),
            aiWaterClient_1.aiWaterClient.leakLocalization(sensorData),
        ]);
        ...
        const sensorParameters = Object.entries(anomalyResult.metrics || {}).map(([name, metric]) => ({
        ...
        const prediction = {
            ...
            stabilityPercent: Math.min(100, Math.max(0, Math.round((ttfResult.remaining_useful_life / 2000) * 100))),
        };
        // Leak localization
        const leakLocation = {
...
router.get('/status', async (_req, res) => {
    ...
    try {
        const result = await aiWaterClient_1.aiWaterClient.predictAnomaly(DEFAULT_SENSORS);
...
router.get('/sensors', (_req, res) => {
    if (lastAnomalyResult?.metrics) {
        const sensorParameters = Object.entries(lastAnomalyResult.metrics).map(([name, metric]) => ({
            name: formatParamName(name),
            value: metric.value,
            baseline: metric.baseline,
...
router.get('/violations', (_req, res) => {
    if (lastAnomalyResult?.top_violations) {
        return res.json(lastAnomalyResult.top_violations.map((v) => ({
            parameter: v.parameter,
...
router.get('/predictions', (_req, res) => {
    if (lastTTFResult) {
        return res.json({
            ...
            stabilityPercent: Math.min(100, Math.max(0, Math.round((lastTTFResult.remaining_useful_life / 2000) * 100))),
        });
    }
    res.json({ remainingUsefulLife: 0, stabilityPercent: 0 });
});
router.get('/leak-location', (_req, res) => {
    if (lastLeakResult) {
...
function generateFallbackReport(anomaly, leak, ttf) {
